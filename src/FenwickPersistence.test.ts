...
class TestList<T> extends FenwickList<T> {
    ...
    setValueByIndex(index: number, value: T): void {
        const { segIndex, localIndex } = (this as unknown as { findByIndex(i: number): { segIndex: number; localIndex: number } }).findByIndex(index);
...
describe("Persistence and CoW logic", () => {
    it("Incremental Flush and CoW: single-segment update writes new chunk and preserves others", async () => {
        ...
        const list = new TestList<number>(store, { segmentCount: 4, chunkCount: 4 });
    ...
    it("State reconstruction from metadata", async () => {
        const store = new MemoryStore();
        const listA = new FenwickList<number>(store, { segmentCount: 32, chunkCount: 8 });
        const N = 1000;
    ...
    it("Cache invalidation reloads persisted state (drops unflushed changes)", async () => {
        const store = new MemoryStore();
        const list = new TestList<number>(store, { segmentCount: 8, chunkCount: 4 });
        ...
        // Clear caches (shim clears arrays and chunk cache)
        list.segmentCache.clear();
    ...
    it("Segment split after flush persists and reloads correctly", async () => {
        const store = new MemoryStore();
        const list = new FenwickList<number>(store, { segmentCount: 10, chunkCount: 4 });
    ...
    it("Empty list operations are safe", async () => {
        const store = new MemoryStore();
        const list = new FenwickList<number>(store, { segmentCount: 8, chunkCount: 4 });
