...
const SortedColumns: Record<string, () => OrderedColumnInterface<number>> = {
  FenwickOrderedColumn: () =>
    new FenwickOrderedColumn<number>(new MemoryStore(), { segmentN: 10, chunkN: 10, chunkPrefix: "ochunk_", idPrefix: "oseg_" }),
};

const IndexedColumns: Record<string, () => IndexedColumnInterface<number>> = {
  FenwickColumn: () => new FenwickColumn<number>(new MemoryStore(), { segmentN: 10, chunkN: 10, chunkPrefix: "chunk_", idPrefix: "seg_" }),
};
...
for (const [name, ctor] of Object.entries(IndexedColumns)) {
  describe(`IIndexedColumn (${name})`, () => {
    it("inserts at index and get reflects shifted positions", async () => {
      const col = ctor();
      await col.insert(0, 2);
      await col.insert(0, 1); // [1,2]
      await col.insert(2, 4); // [1,2,4]
      await col.insert(2, 3); // [1,2,3,4]
    ...
    it("range(min,max) returns slice [min,max)", async () => {
      const col = ctor();
      for (const v of [10, 20, 30, 40, 50]) await col.insert(999, v); // always append
    ...
    it("handles gaps with out-of-range get and appending beyond length", async () => {
      ...
      await col.insert(0, 100); // [100]
      await col.insert(1, 200); // [100,200]
      await col.insert(1, 150); // [100,150,200]
      expect(await col.get(10)).toBeUndefined();

      await col.insert(100, 300); // append -> [100,150,200,300]
    ...
    it("inserting in the middle shifts subsequent elements", async () => {
      const col = ctor();
      for (const v of [1, 3, 4]) await col.insert(999, v); // [1,3,4]
      await col.insert(1, 2); // -> [1,2,3,4]
      expect(await col.get(0)).toBe(1);
      expect(await col.get(1)).toBe(2);
      expect(await col.get(2)).toBe(3);
      expect(await col.get(3)).toBe(4);
    ...
    it("range with min==max and min>max returns empty slice", async () => {
      const col = ctor();
      for (const v of [10, 20, 30]) await col.insert(999, v);
